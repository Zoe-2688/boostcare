...
export default function Badge({ level = 'high', children }) {
  ...
  return (
    <span
      role="img"
      aria-label={`Adherencia ${level === 'high' ? 'alta' : level === 'medium' ? 'media' : 'baja'}`}
      className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold ${classes}`}
    >
      {symbol} {children}
    </span>
  )
